...
test.describe('with the welcome card already dismissed', () => {
  // Every other test exercises the app past its first-run state, so seed the
  // welcome flag before each navigation — otherwise the welcome modal blocks
  // every interaction until dismissed. addInitScript re-runs on every
  // navigation in the page's lifetime, including page.reload(), so it survives
  // the deliberate localStorage.clear() a couple of these tests perform.
  test.beforeEach(async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('gallery-designer-welcome-v1', JSON.stringify({ seen: true }));
    });
  });
  ...
  test('keeps the wall corner action visible and clickable after leaving the artwork', async ({
    ...
    const removeButton = page.getByRole('button', {
      name: 'Return Piece 1 to staging',
      exact: true,
    });
    ...
    if (!wallPieceBox || !removeButtonBox) {
      throw new Error(
        'Placed artwork or its wall remove control was not visible enough to measure.',
      );
    }
    expect(
      Math.abs(
        removeButtonBox.x + removeButtonBox.width / 2 - (wallPieceBox.x + wallPieceBox.width),
      ),
  ...
  test('opens the keyboard shortcuts legend with "?" and closes it with Escape', async ({
    page,
  }) => {
    await page.goto('/');
...
test('shows the welcome card once, remembers dismissal across a reload, and can be reopened', async ({
  ...
  const welcomeCard = page.getByRole('dialog', { name: 'Welcome to Gallery Designer' });
